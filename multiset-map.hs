...
{-
 ...
 -}
inserir elem bag | member elem bag = insertWith (+) elem 1 bag
                 | otherwise = insert elem 1 bag
...
-}
decrementa = \key x -> x - 1

remove elem bag | bag == empty = empty
                | notMember elem bag = bag
                | bag ! elem > 1 = adjustWithKey decrementa elem bag
                | otherwise = delete elem bag
